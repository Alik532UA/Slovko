...
| Гейт над емулятором | `npm run check:rules` — **39 перевірок: 15 дозволено, 24 заборонено** (прогін 2026-08-18) |
| Гейт у CI? | **так**, окремий джоб `rules` у `deploy.yml` із `setup-java` |
| Викладання | `firebase deploy --only firestore:rules,database` |
| Стан консолі звірений із файлом? | **так, 2026-08-18** — живим зондом без токена: `/users`, `/profiles` і неіснуюча колекція віддають `403`, корінь RTDB — `401`. Це стан ПІСЛЯ викладання; що там було до нього, з'ясувати вже неможливо |
