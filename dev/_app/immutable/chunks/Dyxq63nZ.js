const n=`{
	"abundance": "αφθονία",
	"clarity": "σαφήνεια",
	"collaboration": "συνεργασία",
	"collision": "σύγκρουση",
	"commodity": "εμπόρευμα",
	"competence": "ικανότητα",
	"complexity": "πολυπλοκότητα",
	"dominance": "κυριαρχία",
	"abortion": "έκτρωση",
	"absence": "απουσία",
	"acceptance": "αποδοχή",
	"accordance": "συμφωνία",
	"accountability": "λογοδοσία",
	"accumulation": "συσσώρευση",
	"accusation": "κατηγορία",
	"acquisition": "απόκτηση",
	"activation": "ενεργοποίηση",
	"actuality": "πραγματικότητα",
	"adaptation": "προσαρμογή",
	"adherence": "προσκόλληση",
	"admiration": "θαυμασμός",
	"adoration": "λατρεία",
	"affection": "στοργή",
	"affliction": "ταλαιπωρία",
	"aggravation": "επιδείνωση",
	"aggression": "επιθετικότητα",
	"agitation": "αναταραχή",
	"alliance": "συμμαχία",
	"ambiguity": "ασάφεια",
	"amenity": "άνεση",
	"ammunition": "πυρομαχικά",
	"annihilation": "εκμηδένιση",
	"annotation": "σχολιασμός",
	"annuity": "ισόβια πρόσοδος",
	"anonymity": "ανωνυμία",
	"antagonism": "ανταγωνισμός",
	"antiquity": "αρχαιότητα",
	"aphorism": "αφορισμός",
	"apparition": "φάντασμα",
	"appellation": "ονομασία",
	"appliance": "συσκευή",
	"appreciation": "εκτίμηση",
	"aspiration": "φιλοδοξία",
	"assassination": "δολοφονία",
	"assertion": "ισχυρισμός",
	"assurance": "διαβεβαίωση",
	"atrocity": "θηριωδία",
	"attendance": "παρουσία",
	"availability": "διαθεσιμότητα",
	"calculation": "υπολογισμός",
	"capability": "ικανότητα",
	"capitalism": "καπιταλισμός",
	"caution": "προσοχή",
	"cemetery": "νεκροταφείο",
	"circulation": "κυκλοφορία",
	"citizenship": "ιθαγένεια",
	"classification": "ταξινόμηση",
	"coalition": "συνασπισμός",
	"compassion": "συμπόνια",
	"compensation": "αποζημίωση",
	"compliance": "συμμόρφωση",
	"complication": "επιπλοκή",
	"composition": "σύνθεση",
	"conception": "σύλληψη",
	"concession": "παραχώρηση",
	"confession": "ομολογία",
	"configuration": "διαμόρφωση",
	"confirmation": "επιβεβαίωση",
	"confrontation": "αντιπαράθεση",
	"congregation": "εκκλησίασμα",
	"conscience": "συνείδηση",
	"consciousness": "συνείδηση",
	"constitution": "σύνταγμα",
	"consultation": "διαβούλευση",
	"contention": "ισχυρισμός",
	"contradiction": "αντίφαση",
	"conversion": "μετατροπή",
	"coordination": "συντονισμός",
	"correction": "διόρθωση",
	"correlation": "συσχέτιση",
	"correspondence": "αλληλογραφία",
	"corruption": "διαφθορά",
	"credibility": "αξιοπιστία",
	"declaration": "δήλωση",
	"dedication": "αφοσίωση",
	"deduction": "συμπέρασμα",
	"delegation": "αποστολή",
	"density": "πυκνότητα",
	"dependence": "εξάρτηση",
	"detection": "ανίχνευση",
	"detention": "κράτηση",
	"dignity": "αξιοπρέπεια",
	"dimension": "διάσταση",
	"discretion": "διακριτικότητα",
	"discrimination": "διάκριση",
	"disruption": "διακοπή",
	"distinction": "διάκριση",
	"documentation": "τεκμηρίωση",
	"effectiveness": "αποτελεσματικότητα",
	"elevation": "ανύψωση",
	"elimination": "εξάλειψη",
	"emergence": "εμφάνιση",
	"entity": "οντότητα",
	"equality": "ισότητα",
	"erosion": "διάβρωση",
	"essence": "ουσία",
	"excellence": "αριστεία",
	"exclusion": "αποκλεισμός",
	"execution": "εκτέλεση",
	"exploitation": "εκμετάλλευση",
	"faction": "φράξια",
	"fairness": "δικαιοσύνη",
	"flexibility": "ευελιξία",
	"frustration": "απογοήτευση",
	"glance": "ματιά",
	"governance": "διακυβέρνηση",
	"gravity": "βαρύτητα",
	"guidance": "καθοδήγηση",
	"hostility": "εχθρότητα",
	"humanity": "ανθρωπότητα",
	"ignorance": "άγνοια",
	"imagery": "εικόνες",
	"implementation": "εφαρμογή",
	"inability": "ανικανότητα",
	"incarceration": "εγκλεισμός",
	"incidence": "συχνότητα εμφάνισης",
	"inclusion": "ένταξη",
	"inequality": "ανισότητα",
	"insertion": "εισαγωγή",
	"inspection": "επιθεώρηση",
	"inspiration": "έμπνευση",
	"integration": "ενσωμάτωση",
	"integrity": "ακεραιότητα",
	"intensity": "ένταση",
	"interference": "παρεμβολή",
	"intersection": "διασταύρωση",
	"intervention": "παρέμβαση",
	"isolation": "απομόνωση",
	"junction": "διακλάδωση",
	"jurisdiction": "δικαιοδοσία",
	"justification": "αιτιολόγηση",
	"kingdom": "βασίλειο",
	"legislation": "νομοθεσία",
	"liberation": "απελευθέρωση",
	"likelihood": "πιθανότητα",
	"machinery": "μηχανήματα",
	"maintenance": "συντήρηση",
	"manipulation": "χειραγώγηση",
	"meditation": "διαλογισμός",
	"migration": "μετανάστευση",
	"misery": "δυστυχία",
	"mobility": "κινητικότητα",
	"modification": "τροποποίηση",
	"morality": "ηθική",
	"nomination": "υποψηφιότητα",
	"nursery": "βρεφονηπιακός σταθμός",
	"objection": "ένσταση",
	"obsession": "εμμονή",
	"occurrence": "γεγονός",
	"optimism": "αισιοδοξία",
	"orientation": "προσανατολισμός",
	"petition": "αναφορά",
	"precision": "ακρίβεια",
	"preservation": "διατήρηση",
	"prevalence": "επικράτηση",
	"prevention": "πρόληψη",
	"privatization": "ιδιωτικοποίηση",
	"productivity": "παραγωγικότητα",
	"projection": "προβολή",
	"proposition": "πρόταση",
	"prosecution": "δίωξη",
	"prosperity": "ευημερία",
	"provision": "παροχή",
	"realization": "συνειδητοποίηση",
	"reconstruction": "ανασυγκρότηση",
	"reflection": "αντανάκλαση",
	"rehabilitation": "αποκατάσταση",
	"rejection": "απόρριψη",
	"relevance": "συνάφεια",
	"reliability": "αξιοπιστία",
	"representation": "εκπροσώπηση",
	"reproduction": "αναπαραγωγή",
	"residence": "κατοικία",
	"resistance": "αντίσταση",
	"restoration": "αποκατάσταση",
	"revelation": "αποκάλυψη",
	"rotation": "περιστροφή",
	"seldom": "σπάνια",
	"sensation": "αίσθηση",
	"sensitivity": "ευαισθησία",
	"separation": "χωρισμός",
	"sexuality": "σεξουαλικότητα",
	"simulation": "προσομοίωση",
	"slavery": "δουλεία",
	"solidarity": "αλληλεγγύη",
	"specification": "προδιαγραφή",
	"stability": "σταθερότητα",
	"submission": "υποβολή",
	"subscription": "συνδρομή",
	"substitution": "αντικατάσταση",
	"succession": "διαδοχή",
	"supervision": "επίβλεψη",
	"surveillance": "επιτήρηση",
	"tolerance": "ανοχή",
	"transaction": "συναλλαγή",
	"transformation": "μετασχηματισμός",
	"tuition": "δίδακτρα",
	"utility": "χρησιμότητα",
	"validity": "εγκυρότητα",
	"violation": "παραβίαση",
	"vulnerability": "τρωτότητα",
	"willingness": "προθυμία",
	"worship": "λατρεία"
}
`;export{n as default};
