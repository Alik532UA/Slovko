...
const missingOxford = {B2: [], C1: []};
for (const [word, lvl] of Object.entries(oxfordLabeled)) {
    if (!currentSlovko[word]) {
        if (lvl === 'C1') missingOxford.C1.push(word);
        if (lvl === 'B2') missingOxford.B2.push(word);
    }
}
...
function updateMasterList(level, newWords) {
...
updateMasterList('C1', missingOxford.C1);
updateMasterList('B2', missingOxford.B2);

const finalC2 = Array.from(extraWords).sort().slice(0, 300);
fs.writeFileSync('src/lib/data/words/levels/C2.json', JSON.stringify({id: 'C2', words: finalC2}, null, '\t') + '\n', 'utf8');

console.log('Fixed distribution. C1 Missing added:', missingOxford.C1.length, 'Bail in C1:', missingOxford.C1.includes('bail'));
console.log('C2 filled with Truly Extra words:', finalC2.length);
