        ...
        if (fs.statSync(fullPath).isDirectory()) {
            ...
            for (const [key, value] of Object.entries(content)) {
                ...
                if (lang !== 'en' && enValue && value === enValue) {
                    ...
                    const exceptions = [
                        'Wi-Fi', 'OK', 'SMS', 'Taxi', 'Hotel', 'Radio', 'Pizza', 'Pasta', 'Sushi', 
                        'Logo', 'Blog', 'Email', 'Internet', 'Bank', 'Park', 'Metal', 'Million', 
                        'Model', 'Opera', 'Pilot', 'Plan', 'Sport', 'Supermarket', 'Tennis', 
                        'Video', 'Terminal', 'Agent', 'Drama', 'Alibi', 'Zebra', 'Panda', 'Koala',
                        'Server', 'Virus', 'Router', 'Modem', 'USB', 'Disk', 'Festival', 'Normal'
                    ];
                    
                    const isException = exceptions.some(ex => value.toLowerCase() === ex.toLowerCase());
