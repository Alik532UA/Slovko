...
async function fixB2Gaps() {
    console.log(`Starting fix for ${targetLevel} using data from other levels...`);

    // Load B2 data
    const b2WordsFile = path.join(rootDir, `src/lib/data/words/levels/${targetLevel}.json`);
    ...
    for (const lang of languages) {
        ...
        for (const lvl of levels) {
            ...
            const file = path.join(rootDir, `src/lib/data/translations/${lang}/levels/${lvl}.json`);
    ...
    for (const lvl of levels) {
        ...
        const file = path.join(rootDir, `src/lib/data/transcriptions/levels/${lvl}.json`);
        const data = loadJson(file);
    ...
    for (const lang of languages) {
        const b2TransFile = path.join(rootDir, `src/lib/data/translations/${lang}/levels/${targetLevel}.json`);
        ...
        for (const word of b2Words) {
            ...
            const isPlaceholder = currentVal === word.replace(/_/g, ' ') || currentVal === word;
            
            if (!currentVal || currentVal === "" || (lang !== 'en' && isPlaceholder)) {
        ...
        if (updatedCount > 0) {
            ...
            console.log(`Updated ${updatedCount} translations for ${lang} in ${targetLevel}`);
        }
    }

    // Process Transcriptions
    const b2TranscFile = path.join(rootDir, `src/lib/data/transcriptions/levels/${targetLevel}.json`);
    ...
    if (transUpdatedCount > 0) {
        ...
        console.log(`Updated ${transUpdatedCount} transcriptions in ${targetLevel}`);
    }
}
