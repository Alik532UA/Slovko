...
files.forEach((file) => {
	const content = JSON.parse(
		fs.readFileSync(path.join(enPhrasesDir, file), "utf8"),
	);
...
const existingMapping = JSON.parse(
	fs.readFileSync("polysemy_migration.json", "utf8"),
);
const finalMapping = { ...existingMapping, ...phraseMapping };

fs.writeFileSync(
	"polysemy_migration.json",
	JSON.stringify(finalMapping, null, 4),
);
console.log(
	`Added ${Object.keys(phraseMapping).length} phrases to migration map.`,
);
