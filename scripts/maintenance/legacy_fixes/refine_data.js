...
async function refine() {
	...
	for (const file of levelFiles) {
		const content = JSON.parse(
			await fs.readFile(path.join(levelsDir, file), "utf-8"),
		);
	...
	for (const file of topicFiles) {
		...
		const content = JSON.parse(
			await fs.readFile(path.join(topicsDir, file), "utf-8"),
		);
	...
	for (const lang of languages) {
		...
		const allLevelWords = new Set(
			Object.values(levelWords).flatMap((s) => [...s]),
		);
		const allTopicWords = new Set(
			Object.values(topicWords).flatMap((s) => [...s]),
		);
