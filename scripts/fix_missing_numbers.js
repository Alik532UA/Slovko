...
const NEW_WORDS = [
    ["sixteen", "sixteen", "шістнадцять", "on altı", "sechzehn", " zestien", "ˌsɪksˈtiːn"],
    ["seventeen", "seventeen", "сімнадцять", "on yedi", "siebzehn", "zeventien", "ˌsevnˈtiːn"],
    ["eighteen", "eighteen", "вісімнадцять", "on sekiz", "achtzehn", "achttien", "ˌeɪˈtiːn"],
    ["nineteen", "nineteen", "дев'ятнадцять", "on doquz", "neunzehn", "negentien", "ˌnaɪnˈtiːn"],
    ["sixty", "sixty", "шістдесят", "altmış", "sechzig", "zestig", "ˈsɪksti"],
    ["seventy", "seventy", "сімдесят", "yetmiş", "siebzig", "zeventig", "ˈsevnti"],
    ["eighty", "eighty", "вісімдесят", "seksen", "achtzig", "tachtig", "ˈeɪti"],
    ["ninety", "ninety", "дев'яносто", "doqsan", "neunzig", "negentig", "ˈnaɪnti"]
];

function update() {
    const wordsPath = path.join(DATA_DIR, 'words/levels', `${LEVEL}.json`);
    const wordsData = JSON.parse(fs.readFileSync(wordsPath, 'utf-8'));
    NEW_WORDS.forEach(([key]) => { if (!wordsData.words.includes(key)) wordsData.words.push(key); });
    ...
    ['en', 'uk', 'crh', 'de', 'nl'].forEach((lang, idx) => {
        ...
        NEW_WORDS.forEach(([key, ...translations]) => { transData[key] = translations[idx]; });
    ...
    NEW_WORDS.forEach(([key, , , , , , ipa]) => { ipaData[key] = ipa; });
