...
if (args.length < 2) {
    console.error('Usage: node scripts/analyze_words.js <txtFile> <level> [outFile]');
    process.exit(1);
}
...
function getParts(str) {
    if (!str) return [];
    return str.toLowerCase().split(/[\/;()]/).map(s => s.trim()).filter(s => s);
}
...
lines.forEach(line => {
    ...
    if (foundKey) {
        ...
        missing.push({ en: enRaw, uk: ukRaw, type: matchType, candidates: Array.from(candidates) });
    }
});
