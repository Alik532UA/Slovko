			...
			if (item.topics) {
				item.topics.forEach((topic) => {
					...
					if (fs.existsSync(topicTransPath)) {
						const topicTransData = JSON.parse(
							fs.readFileSync(topicTransPath, "utf8"),
						);
						topicTransData[item.key] = item.translations[lang];
						fs.writeFileSync(
							topicTransPath,
							JSON.stringify(topicTransData, null, 4),
						);
					}
				});
			}
		...
		if (item.topics) {
			item.topics.forEach((topic) => {
				...
				if (fs.existsSync(topicTranscPath)) {
					const topicTranscData = JSON.parse(
						fs.readFileSync(topicTranscPath, "utf8"),
					);
					topicTranscData[item.key] = item.transcription;
					fs.writeFileSync(
						topicTranscPath,
						JSON.stringify(topicTranscData, null, 4),
					);
				}
			});
		}
