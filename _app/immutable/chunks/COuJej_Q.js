const n=`{
	"account_for": "αιτιολογώ",
	"account_for_verb": "αιτιολογώ",
	"advance": "πρόοδος",
	"allocation": "κατανομή πόρων",
	"allocation_resources": "κατανομή πόρων",
	"alteration": "αλλοίωση",
	"anger": "θυμός",
	"angle": "γωνία",
	"anticipation": "προσδοκία",
	"anxiety": "άγχος",
	"aspect": "πτυχή",
	"asset": "περιουσιακό στοιχείο",
	"assistance": "βοήθεια",
	"association": "σύλλογος",
	"assumption": "υπόθεση",
	"awareness": "επίγνωση",
	"capacity": "χωρητικότητα",
	"circumstance": "περίσταση",
	"cohesion": "συνοχή",
	"commitment_dedication": "αφοσίωση",
	"comprehension": "κατανόηση",
	"conflict": "σύγκρουση",
	"conformity": "συμμόρφωση",
	"confusion": "σύγχυση",
	"consideration_thought": "σκέψη",
	"construction": "κατασκευή",
	"construction_noun": "κατασκευή",
	"consumption": "κατανάλωση",
	"contribution": "συνεισφορά",
	"contribution_help": "συνεισφορά",
	"convenience": "ευκολία",
	"convention_norm": "σύμβαση",
	"conviction": "πεποίθηση",
	"conviction_belief": "πεποίθηση",
	"cooperation": "συνεργασία",
	"cooperation_noun": "συνεργασία",
	"criticism": "κριτική",
	"curiosity": "περιέργεια",
	"curiosity_interest": "περιέργεια",
	"deception": "εξαπάτηση",
	"depiction": "απεικόνιση",
	"depression_mood": "κατάθλιψη",
	"designation": "ονομασία",
	"edition": "έκδοση",
	"elaboration": "επεξεργασία",
	"implication": "συνέπεια",
	"indication": "ένδειξη",
	"majority": "πλειοψηφία",
	"minority": "μειοψηφία",
	"mission": "αποστολή",
	"partnership": "συνεργασία",
	"protection": "προστασία",
	"query": "ρωτώ",
	"query_question": "ρωτώ",
	"reputation": "φήμη",
	"sanction": "επιβάλλω κυρώσεις",
	"sanction_allow": "επιβάλλω κυρώσεις",
	"sponsorship": "χορηγία",
	"supplement": "συμπληρώνω",
	"supplement_add": "συμπληρώνω",
	"violence": "βία",
	"aberration": "παρέκκλιση",
	"aberration_deviation": "παρέκκλιση",
	"abnegation": "απάρνηση",
	"absolution": "άφεση",
	"accretion": "συσσώρευση",
	"accretion_buildup": "ανάπτυξη",
	"accretion_growth": "συσσώρευση",
	"acknowledgement_confirmation": "επιβεβαίωση",
	"acknowledgement_recognition": "αναγνώριση",
	"addiction": "εθισμός",
	"adjustment_change": "προσαρμογή",
	"admission": "είσοδος",
	"admission_kiri": "είσοδος",
	"adoption": "υιοθεσία",
	"adoption_adoptie": "υιοθεσία",
	"adulation": "κολακεία",
	"adversity": "αντιξοότητα",
	"adversity_hardship": "δυστυχία",
	"adversity_misfortune": "αντιξοότητα",
	"affectation": "επιτήδευση",
	"affinity": "συγγένεια",
	"affinity_affinity": "συγγένεια",
	"affinity_yaqnlq": "ελκυστικότητα",
	"agility": "ευκινησία",
	"alacrity": "προθυμία",
	"alacrity_canllq": "προθυμία",
	"allegation": "ισχυρισμός",
	"allegation_iddia": "ισχυρισμός",
	"allegiance": "πίστη",
	"allowance": "επίδομα",
	"allowance_maa": "επίδομα",
	"allowance_money": "ανοχή",
	"allusion": "υπαινιγμός",
	"allusion_ima": "υπαινιγμός",
	"altercation": "λογομαχία",
	"altruism": "αλτρουισμός",
	"ambience": "περιβάλλον",
	"amity": "φιλία",
	"anachronism": "αναχρονισμός",
	"animation": "κινούμενα σχέδια",
	"animosity": "εχθρότητα",
	"approbation": "έγκριση",
	"auction": "δημοπρασία",
	"championship": "πρωτάθλημα",
	"civilization": "πολιτισμός",
	"coincidence": "σύμπτωση",
	"combination": "συνδυασμός",
	"commence": "ξεκινώ",
	"commence_ceremony": "ξεκινώ",
	"commence_start": "ξεκινώ",
	"commission": "επιτροπή",
	"complement_style": "συμπληρώνω",
	"completion": "ολοκλήρωση",
	"concentration": "συγκέντρωση",
	"conservation": "διατήρηση",
	"consideration": "σκέψη",
	"convention": "σύμβαση",
	"corporation": "εταιρεία",
	"creation": "δημιουργία",
	"creativity": "δημιουργικότητα",
	"darkness": "σκοτάδι",
	"decoration": "διακόσμηση",
	"defence": "άμυνα",
	"delivery": "παράδοση",
	"demonstration": "επίδειξη",
	"demonstration_noun": "επίδειξη",
	"depression": "κατάθλιψη",
	"destruction": "καταστροφή",
	"determination": "αποφασιστικότητα",
	"disability": "αναπηρία",
	"distribution": "διανομή",
	"diversity": "ποικιλομορφία",
	"division": "διαίρεση",
	"donation": "δωρεά",
	"duration": "διάρκεια",
	"emission": "εκπομπή",
	"enhance": "βελτιώνω",
	"enhance_improve": "βελτιώνω",
	"enhance_quality": "βελτιώνω την ποιότητα",
	"enhancement_augmentation": "ενίσχυση",
	"envision": "οραματίζομαι το μέλλον",
	"envision_future": "οραματίζομαι το μέλλον",
	"envision_imagine": "οραματίζομαι",
	"equation": "εξίσωση",
	"equation_noun": "εξίσωση",
	"establishment_institution": "ίδρυμα",
	"evaluation": "αξιολόγηση",
	"evolution": "εξέλιξη",
	"examination": "εξέταση",
	"exception": "εξαίρεση",
	"existence": "ύπαρξη",
	"expansion": "επέκταση",
	"expectation": "προσδοκία",
	"exploration": "εξερεύνηση",
	"extension": "επέκταση",
	"facility": "εγκατάσταση",
	"facility_amenity": "παροχή",
	"finance": "οικονομικά",
	"formation": "σχηματισμός",
	"foundation": "θεμέλιο",
	"foundation_base": "θεμέλιο",
	"foundation_organization": "ίδρυμα",
	"fraction": "κλάσμα",
	"fraction_math": "κλάσμα",
	"fraction_part": "τμήμα",
	"globalization": "παγκοσμιοποίηση",
	"goodness": "καλοσύνη",
	"hence": "ως εκ τούτου",
	"illusion": "ψευδαίσθηση",
	"illustration": "εικονογράφηση",
	"independence": "ανεξαρτησία",
	"inflation": "πληθωρισμός",
	"innovation": "καινοτομία",
	"installation": "εγκατάσταση",
	"instance": "περίπτωση",
	"institution": "ίδρυμα",
	"interaction": "αλληλεπίδραση",
	"interpretation": "ερμηνεία",
	"invasion": "εισβολή",
	"investigation": "έρευνα",
	"journalism": "δημοσιογραφία",
	"leadership": "ηγεσία",
	"licence": "άδεια",
	"limitation": "περιορισμός",
	"lottery": "λοταρία",
	"mechanism": "μηχανισμός",
	"membership": "ιδιότητα μέλους",
	"motion": "κίνηση",
	"motivation": "κίνητρο",
	"navigation": "πλοήγηση",
	"necessity": "αναγκαιότητα",
	"notion": "έννοια",
	"nutrition": "διατροφή",
	"obligation": "υποχρέωση",
	"observation": "παρατήρηση",
	"occupation": "επάγγελμα / κατοχή",
	"occupation_job": "επάγγελμα",
	"offence": "αδίκημα",
	"opposition": "αντιπολίτευση",
	"ownership": "ιδιοκτησία",
	"participation": "συμμετοχή",
	"pension": "σύνταξη",
	"perception": "αντίληψη",
	"pity": "κρίμα",
	"popularity": "δημοτικότητα",
	"portion": "μερίδα",
	"preference": "προτίμηση",
	"preparation": "προετοιμασία",
	"presence": "παρουσία",
	"probability": "πιθανότητα",
	"proportion": "αναλογία",
	"publication": "δημοσίευση",
	"publicity": "δημοσιότητα",
	"racism": "ρατσισμός",
	"radiation": "ακτινοβολία",
	"random": "τυχαίος",
	"recession": "ύφεση",
	"recognition": "αναγνώριση",
	"recovery": "ανάκαμψη",
	"reduction": "μείωση",
	"registration": "εγγραφή",
	"regulation": "κανονισμός",
	"resolution": "απόφαση",
	"resolution_decision": "απόφαση",
	"restriction": "περιορισμός",
	"revision": "αναθεώρηση",
	"revolution": "επανάσταση",
	"robbery": "ληστεία",
	"romance": "ρομάντζο",
	"satisfaction": "ικανοποίηση",
	"scale": "κλίμακα",
	"scholarship": "υποτροφία",
	"sequence": "ακολουθία",
	"session": "συνεδρία",
	"significance": "σημασία",
	"speculation": "εικασία",
	"stance": "στάση",
	"tension": "ένταση",
	"terrorism": "τρομοκρατία",
	"transition": "μετάβαση",
	"unity": "ενότητα",
	"variation": "παραλλαγή",
	"vision": "όραμα",
	"wisdom": "σοφία"
}
`;export{n as default};
