const t=`{
	"afterwards": "μετά",
	"besides": "επιπλέον",
	"consequently": "κατά συνέπεια",
	"entirely": "εντελώς",
	"formerly": "πρώην",
	"formerly_past": "κάποτε",
	"formerly_previously": "προηγουμένως",
	"gradually": "σταδιακά",
	"monthly": "μηνιαία",
	"moreover": "επιπλέον",
	"nevertheless": "παρόλα αυτά",
	"occasionally": "περιστασιακά",
	"occasionally_rarely": "σπάνια",
	"occasionally_sometimes": "κάπου-κάπου",
	"otherwise": "διαφορετικά",
	"terribly": "φρικτά",
	"truly": "πραγματικά",
	"weekly": "εβδομαδιαία",
	"whereas": "ενώ",
	"accidentally": "τυχαία",
	"accurately": "με ακρίβεια",
	"additionally": "επιπλέον",
	"adequately": "επαρκώς",
	"advocate_advocate": "συνήγορος",
	"annually": "ετησίως",
	"apparently": "προφανώς",
	"appropriately": "κατάλληλα",
	"barely": "μόλις",
	"basically": "βασικά",
	"briefly": "σύντομα",
	"broadly": "ευρέως",
	"closely": "στενά",
	"commonly": "συνήθως",
	"considerably": "σημαντικά",
	"consistently": "συνεπώς",
	"constantly": "συνεχώς",
	"critically": "κρίσιμα",
	"deadly": "θανάσιμος",
	"deeply": "βαθιά",
	"deliberately": "εσκεμμένα",
	"desperately": "απελπισμένα",
	"dramatically": "δραματικά",
	"efficiently": "αποτελεσματικά",
	"elderly": "ηλικιωμένος",
	"emotionally": "συναισθηματικά",
	"essentially": "ουσιαστικά",
	"extensively": "εκτενώς",
	"firmly": "σταθερά",
	"freely": "ελεύθερα",
	"fully": "πλήρως",
	"fundamentally": "θεμελιωδώς",
	"genuinely": "πραγματικά",
	"greatly": "πολύ",
	"holy": "άγιος",
	"hopefully": "ελπίζω",
	"imply": "υπονοώ",
	"increasingly": "όλο και περισσότερο",
	"inevitably": "αναπόφευκτα",
	"initially": "αρχικά",
	"instantly": "αμέσως",
	"largely": "κυρίως",
	"lately": "τελευταία",
	"literally": "κυριολεκτικά",
	"lively": "ζωντανός",
	"multiply": "πολλαπλασιάζω",
	"newly": "πρόσφατα",
	"openly": "ανοιχτά",
	"partly": "εν μέρει",
	"permanently": "μόνιμα",
	"potentially": "δυνητικά",
	"precisely": "ακριβώς",
	"primarily": "πρωτίστως",
	"purely": "καθαρά",
	"rapidly": "ταχέως",
	"reasonably": "λογικά",
	"relatively": "σχετικά",
	"remarkably": "αξιοσημείωτα",
	"roughly": "περίπου",
	"severely": "σοβαρά",
	"shortly": "σύντομα",
	"significantly": "σημαντικά",
	"steadily": "σταθερά",
	"strictly": "αυστηρά",
	"subsequently": "στη συνέχεια",
	"sufficiently": "επαρκώς",
	"temporarily": "προσωρινά",
	"thoroughly": "διεξοδικά",
	"ultimately": "τελικά",
	"widely": "ευρέως"
}
`;export{t as default};
