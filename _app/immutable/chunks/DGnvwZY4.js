const t=`\uFEFF{
	"able": "здібний",
	"abroad": "за кордоном",
	"according": "згідно з",
	"according_to": "згідно з",
	"achieve": "досягати",
	"active": "активний",
	"actually": "насправді",
	"adventure": "пригода",
	"advert": "оголошення",
	"advertising": "реклама",
	"affect": "впливати",
	"ah": "а!",
	"ah_exclam.": "а!",
	"airline": "авіакомпанія",
	"alive": "живий",
	"all_right": "гаразд",
	"allow": "дозволяти",
	"almost": "майже",
	"alone": "сам",
	"already": "вже",
	"alright": "гаразд",
	"alternative": "альтернатива",
	"ankle": "щиколотка",
	"any_more": "більше не",
	"anymore": "більше не",
	"appear": "з'являтися",
	"appearance": "зовнішність",
	"appearance_arrival": "поява",
	"appearance_look": "зовнішність",
	"are": "є (мн.)",
	"argue": "сперечатися",
	"argue_dispute": "сперечатися",
	"argue_reason": "аргументувати",
	"armchair": "крісло",
	"arrange": "організовувати",
	"arrangement": "домовленість",
	"as_well": "також",
	"athlete": "спортсмен",
	"available": "доступний",
	"awful": "жахливий",
	"background": "фон",
	"bad_luck": "невдача",
	"bar": "бар / засув",
	"bar_rod": "стержень",
	"bar_tavern": "бар",
	"based": "заснований",
	"bean": "квасоля",
	"bear": "ведмідь",
	"bear_animal": "ведмідь",
	"bear_endure": "терпіти",
	"beard": "борода",
	"beef": "яловичина",
	"behaviour": "поведінка",
	"belong": "належати",
	"billion": "мільярд",
	"bin": "смітник",
	"biology": "біологія",
	"biscuit": "печиво",
	"blow": "дути",
	"borrow": "позичати",
	"bottom": "дно",
	"branch": "гілка",
	"branch_office": "філія / відділення",
	"branch_tak": "гілка",
	"branch_tree": "гілка",
	"bridge": "міст",
	"bright": "яскравий",
	"bug": "жук",
	"bug_error": "помилка",
	"bug_insect": "жук",
	"builder": "будівельник",
	"burn": "горіти",
	"burn_fire": "горіти",
	"burn_injury": "опік",
	"businessman": "бізнесмен",
	"button": "кнопка",
	"buyer": "покупець",
	"calendar": "календар",
	"camping": "кемпінг",
	"campus": "кампус",
	"candy": "цукерка",
	"careful": "обережний",
	"cartoon": "мультфільм",
	"castle": "замок",
	"catch": "ловити",
	"celebrate": "святкувати",
	"celebrity": "знаменитість",
	"cell": "клітина",
	"cell_biology": "клітина",
	"cell_phone": "стільниковий",
	"cell_prison": "камера",
	"certain": "певний",
	"chemist": "хімік",
	"chip": "чип",
	"church": "церква",
	"cigarette": "сигарета",
	"classical": "класичний",
	"clerk": "клерк",
	"clever": "розумний",
	"closed": "закритий",
	"closet": "шафа",
	"clothing": "одяг",
	"cloudy": "хмарний",
	"colleague": "колега",
	"collect": "збирати",
	"column": "колонка",
	"comedy": "комедія",
	"comfortable": "зручний",
	"communicate": "спілкуватися",
	"compete": "змагатися",
	"complain": "скаржитися",
	"connect": "з'єднувати",
	"connected": "пов'язаний",
	"context": "контекст",
	"continent": "континент",
	"continue": "продовжувати",
	"cooker": "плита",
	"corner": "кут",
	"count": "рахувати",
	"couple": "пара",
	"cover": "накривати",
	"crazy": "божевільний",
	"creative": "творчий",
	"credit": "кредит",
	"cross": "перетинати",
	"cross_noun": "хрест",
	"cross_symbol": "хрест",
	"cross_verb": "перетинати",
	"crowded": "людний",
	"cry": "плакати",
	"curly": "кучерявий",
	"dead": "мертвий",
	"dear_exclam.": "о боже",
	"death": "смерть",
	"deep": "глибокий",
	"definitely": "безумовно",
	"definitely_certainly": "безумовно",
	"definitely_exactly": "точно",
	"degree_temperature": "градус",
	"dentist": "стоматолог",
	"desert": "пустеля",
	"designer": "дизайнер",
	"dessert": "десерт",
	"destroy": "руйнувати",
	"detective": "детектив",
	"device": "пристрій",
	"diary": "щоденник",
	"differently": "інакше",
	"director": "директор",
	"disagree": "не погоджуватися",
	"discovery": "відкриття",
	"divorced": "розлучений",
	"double": "подвійний",
	"download": "завантажувати",
	"download_file": "завантаження",
	"downtown": "центр міста",
	"drama": "драма",
	"drama_art": "драма",
	"drawing": "малюнок",
	"dream": "мрія",
	"dream_goal": "мрія",
	"dream_sleep": "сон",
	"driving": "водіння",
	"drop": "впускати",
	"drug": "ліки",
	"drug_medicine": "ліки",
	"drug_narcotic": "наркотик",
	"dry": "сухий",
	"earn": "заробляти",
	"earth": "земля",
	"earth_ground": "земля",
	"earth_planet": "земля",
	"earth_yer": "земля",
	"either": "або",
	"electrical": "електричний",
	"electricity": "електрика",
	"elevator": "ліфт",
	"employ": "наймати",
	"employee": "працівник",
	"employer": "роботодавець",
	"ending": "закінчення",
	"energy": "енергія",
	"engine": "двигун",
	"engineer": "інженер",
	"enormous": "величезний",
	"enter": "входити",
	"equipment": "обладнання",
	"error": "помилка",
	"essay": "есе",
	"everyday": "буденна",
	"evidence": "доказ",
	"exact": "точний",
	"excellent": "відмінний",
	"except": "крім",
	"exist": "існувати",
	"expect": "очікувати",
	"experiment": "експеримент",
	"expert": "експерт",
	"explanation": "пояснення",
	"expression": "вираз",
	"extreme": "екстремальний",
	"extremely": "надзвичайно",
	"factor": "фактор",
	"factory": "завод",
	"fail": "провалити",
	"fair": "справедливий",
	"fair_exhibition": "ярмарок",
	"fair_just": "справедливий",
	"farming": "фермерство",
	"fashion": "мода",
	"feed": "годувати",
	"female": "жіночий",
	"fiction": "художня література",
	"field": "поле",
	"fight": "битися",
	"fight_action": "битися",
	"fight_event": "бійка",
	"fireplace": "камін",
	"firstly": "по-перше",
	"fishing": "риболовля",
	"fit": "підходити",
	"fit_healthy": "у формі",
	"fit_shape": "у формі",
	"fit_size": "підходити",
	"fix": "лагодити",
	"flu": "грип",
	"flying": "політ",
	"following": "наступний",
	"foreign": "іноземний",
	"formal": "формальний",
	"fortunately": "на щастя",
	"forward": "вперед",
	"fresh": "свіжий",
	"further": "далі",
	"gallery": "галерея",
	"gap": "розрив",
	"gas": "газ / бензин",
	"gas_fuel": "бензин",
	"gas_gasoline": "бензин",
	"gas_station": "заправка",
	"gas_substance": "газ",
	"gift": "подарунок",
	"god": "бог",
	"golf": "гольф",
	"grocery": "бакалія",
	"ground": "земля",
	"guest": "гість",
	"guidebook": "путівник",
	"gun": "зброя",
	"happily": "щасливо",
	"have_auxiliary": "мати",
	"heat": "спека",
	"heavy": "важкий",
	"height": "висота",
	"helpful": "корисний",
	"hero": "герой",
	"hers": "її",
	"hide": "ховати",
	"hit": "вдарити",
	"hockey": "хокей",
	"hold": "тримати",
	"hold_on": "зачекати",
	"hole": "дірка",
	"huge": "величезний",
	"human": "людина",
	"hurt": "біль / ранити",
	"hurt_injury": "травмувати",
	"hurt_pain": "боліти",
	"ideal": "ідеальний",
	"ill": "хворий",
	"illness": "хвороба",
	"impossible": "неможливий",
	"included": "включений",
	"including": "включаючи",
	"increase": "збільшувати",
	"incredible": "неймовірний",
	"industry": "промисловість",
	"informal": "неформальний",
	"inside": "всередині",
	"instructor": "інструктор",
	"instrument": "інструмент",
	"intelligent": "розумний",
	"introduction": "вступ",
	"invention": "винахід",
	"invitation": "запрошення",
	"invite": "запрошувати",
	"iron": "залізо",
	"iron_device": "праска",
	"iron_metal": "залізо",
	"is": "є (однина)",
	"item": "предмет",
	"jam": "джем",
	"jam_fruit": "варення",
	"jam_traffic": "затор",
	"jazz": "джаз",
	"jewellery": "ювелірні вироби",
	"jewelry": "ювелірні вироби",
	"joke": "жарт",
	"journalist": "журналіст",
	"jump": "стрибати",
	"kill": "вбивати",
	"kilometer": "кілометр",
	"king": "король",
	"lab": "лабораторія",
	"laughter": "сміх",
	"lawyer": "юрист",
	"lazy": "ледачий",
	"lead": "вести",
	"learning": "навчання",
	"least": "найменше",
	"lecture": "лекція",
	"lend": "позичати (комусь)",
	"less": "менше",
	"letter_mail": "лист",
	"lifestyle": "спосіб життя",
	"lift": "ліфт",
	"lift_action": "піднімати",
	"lift_elevator": "ліфт",
	"likely": "ймовірно",
	"listener": "слухач",
	"lock": "замок",
	"lorry": "вантажівка",
	"loud": "гучний",
	"lovely": "чудовий",
	"lucky": "щасливий",
	"mad": "божевільний",
	"mad_angry": "злий",
	"mad_insane": "божевільний",
	"mail": "пошта",
	"male": "чоловічий",
	"manage": "управляти",
	"math": "математика",
	"mathematics": "математика",
	"maths": "математика",
	"matter": "справа",
	"matter_issue": "справа",
	"matter_substance": "матерія",
	"media": "медіа",
	"medicine": "медицина",
	"medicine_drug": "ліки",
	"medicine_field": "медицина",
	"medicine_substance": "ліки",
	"metal": "метал",
	"middle": "середина",
	"might": "може бути",
	"mine": "мій",
	"mine_excavation": "шахта",
	"mine_possessive": "мій",
	"missing": "відсутній",
	"mobile": "мобільний",
	"mobile_phone": "мобільний телефон",
	"mostly": "переважно",
	"musical": "музичний",
	"musician": "музикант",
	"narrow": "вузький",
	"nature": "природа",
	"neither": "ні той, ні інший",
	"neither_conjunction": "також ні",
	"neither_pronoun": "ні той, ні інший",
	"nervous": "нервовий",
	"nicely": "гарно",
	"nicely_kindly": "мило",
	"nicely_well": "гарно",
	"noise": "шум",
	"noisy": "галасливий",
	"none": "Немає",
	"normal": "нормальний",
	"normally": "зазвичай",
	"novel": "роман",
	"nut": "горіх",
	"offer": "пропонувати",
	"offer_noun": "пропозиція",
	"offer_verb": "пропонувати",
	"ordinary": "звичайний",
	"own_property": "володіти",
	"pack": "пакувати",
	"pack_bag": "пакувати",
	"pain": "біль",
	"painter": "художник",
	"palace": "палац",
	"parking": "парковка",
	"pass": "проходити",
	"passenger": "пасажир",
	"penny": "пенні",
	"per_cent": "відсоток",
	"percent": "відсоток",
	"perhaps": "мабуть",
	"personality": "особистість",
	"pet": "свійські тварини",
	"petrol": "бензин",
	"pilot": "пілот",
	"planet": "планета",
	"pleased": "задоволений",
	"pocket": "кишеня",
	"police_station": "поліцейська дільниця",
	"polite": "ввічливий",
	"pop": "поп-музика",
	"possession": "володіння",
	"poster": "плакат",
	"pot": "горщик",
	"practice_skill": "практикувати",
	"prison": "в'язниця",
	"prize": "приз",
	"profile": "профіль",
	"promise": "обіцяти",
	"pronounce": "вимовляти",
	"protect": "захищати",
	"pub": "паб",
	"public": "публічний",
	"public_transport": "громадський транспорт",
	"pull": "тягнути",
	"push": "штовхати",
	"quantity": "кількість",
	"queen": "королева",
	"race": "гонка",
	"railroad": "залізниця",
	"railway": "залізниця",
	"raise": "піднімати",
	"rate": "швидкість",
	"rate_speed": "швидкість",
	"rate_value": "ставка",
	"rather": "радше",
	"reach": "досягати",
	"react": "реагувати",
	"realize": "усвідомлювати",
	"receive": "отримувати",
	"recently": "нещодавно",
	"reception": "прийом",
	"recipe": "рецепт",
	"recognize": "розпізнавати",
	"recommend": "рекомендувати",
	"recording": "запис",
	"recycle": "переробляти",
	"refer": "звертатися",
	"refrigerator": "холодильник",
	"refuse": "відмовлятися",
	"region": "регіон",
	"remove": "видаляти",
	"repair": "ремонтувати",
	"replace": "замінювати",
	"reply": "відповідати",
	"reporter": "репортер",
	"request": "просити",
	"research": "дослідження",
	"researcher": "дослідник",
	"respond": "відповідати",
	"response": "відповідь",
	"rest": "відпочивати",
	"return_ticket": "зворотній квиток",
	"review": "огляд",
	"ring": "кільце / дзвінок",
	"ring_jewel": "кільце",
	"ring_jewelry": "кільце",
	"ring_sound": "дзвонити",
	"rise": "вставати",
	"rock": "скеля",
	"rock_music": "рок-музика",
	"rock_stone": "камінь",
	"round": "круглий",
	"route": "маршрут",
	"rude": "грубий",
	"runner": "бігун",
	"running": "біг",
	"sadly": "на жаль",
	"sail": "вітрило",
	"sailing": "вітрильний спорт",
	"sauce": "соус",
	"save": "зберігати",
	"save_keep": "зберігати",
	"save_rescue": "рятувати",
	"save_store": "зберігати",
	"scared": "наляканий",
	"scary": "страшний",
	"schedule": "розклад",
	"secondly": "по-друге",
	"secretary": "секретар",
	"separate": "окремий",
	"serious": "серйозний",
	"serve": "обслуговувати",
	"serve_duty": "служити",
	"serve_general": "служити",
	"serve_service": "обслуговувати",
	"set": "набір / ставити",
	"set_collection": "набір",
	"set_off_journey": "вирушити",
	"set_up": "налаштувати",
	"set_verb": "встановлювати",
	"several": "декілька",
	"shake": "трусити",
	"shall": "буде (допоміжне дієслово)",
	"shape": "форма",
	"shoes": "взуття",
	"shout": "кричати",
	"shut": "зачиняти",
	"side": "сторона",
	"simple": "простий",
	"singing": "Спів",
	"single_ticket": "квиток в один кінець",
	"sir": "сер",
	"size": "розмір",
	"skiing": "катання на лижах",
	"skin": "шкіра",
	"smartphone": "смартфон",
	"smell": "запах",
	"smell_action": "нюхати",
	"smell_sense": "нюх",
	"smile": "посміхатися",
	"smoke": "палити",
	"smoke_action": "курити",
	"smoke_gas": "дим",
	"smoking": "куріння",
	"sneaker": "кросівок",
	"soccer": "футбол",
	"soft": "м'який",
	"solution": "рішення",
	"solve": "вирішувати",
	"speech": "мова",
	"square": "квадрат",
	"square_place": "площа",
	"square_shape": "квадрат",
	"stair": "сходинка",
	"stamp": "штамп",
	"steal": "красти",
	"store": "магазин",
	"store_shop": "магазин",
	"store_verb": "зберігати",
	"strange": "дивний",
	"stress": "стрес",
	"stupid": "дурний",
	"subway": "метро",
	"succeed": "досягати успіху",
	"successful": "успішний",
	"suddenly": "раптово",
	"suggest": "пропонувати",
	"suggestion": "пропозиція",
	"support": "підтримувати",
	"support_help": "підтримувати",
	"surprise": "сюрприз",
	"surprising": "дивовижний",
	"survey": "опитування",
	"sweet": "солодкий",
	"tablet": "планшет / таблетка",
	"tablet_device": "планшет",
	"tablet_pill": "таблетка",
	"taste": "смак",
	"teaching": "викладання",
	"teenage": "підлітковий",
	"term": "термін",
	"thick": "товстий",
	"thief": "злодій",
	"thin": "худий",
	"thinking": "мислення",
	"throw": "кидати",
	"tidy": "охайний",
	"tip": "порада",
	"tool": "інструмент",
	"top": "верх",
	"touch": "торкатися",
	"touch_noun": "дотик",
	"touch_verb": "торкатися",
	"towel": "рушник",
	"toy": "іграшка",
	"track": "трек / стежка",
	"track_music": "трек",
	"track_path": "стежка",
	"traffic_light": "світлофор",
	"trainer": "тренер",
	"transportation": "транспорт",
	"traveler": "мандрівник",
	"traveller": "мандрівник",
	"twin": "близнюк",
	"typical": "типовий",
	"underground": "підземний",
	"understanding": "розуміння",
	"unhappy": "нещасливий",
	"united": "об'єднаний",
	"unusual": "незвичайний",
	"used": "вживаний",
	"used_to": "раніше",
	"usual": "звичайний",
	"van": "фургон",
	"washing": "прання",
	"web": "павутина",
	"wet": "мокрий",
	"while": "поки",
	"wide": "широкий",
	"wild": "дикий",
	"winner": "переможець",
	"wish": "бажати",
	"wish_noun": "бажання",
	"wish_verb": "бажати",
	"wood": "дерево",
	"wood_bos": "ліс",
	"wood_forest": "ліс",
	"wood_holz": "деревина",
	"wood_material": "деревина",
	"working": "працюючий",
	"worried": "стурбований",
	"worry": "хвилюватися",
	"worse": "гірше",
	"worst": "найгірше",
	"wow": "вау!",
	"wow_exclam.": "ого!",
	"yet": "ще",
	"yours": "твій",
	"zero": "нуль",
	"zoo": "зоопарк",
	"race_competition": "гонка",
	"rest_remaining": "решта",
	"rest_sleep": "відпочинок"
}
`;export{t as default};
