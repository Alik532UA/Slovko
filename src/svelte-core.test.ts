...
/**
 * Запит у мережу без сигналу скасування (`SC-ABORT-SIGNAL`, MEDIUM).
 *
 * `fetch` без `signal` не закінчується сам. «Мережі немає» і «мережа є,
 * відповіді немає» — різні випадки, і другий триває, доки браузер не здасться:
 * captive portal у готелі, мертвий проксі, тунель, вимкнений посеред запиту. У
 * Chrome це десятки секунд, а в частині випадків не настає ніколи.
 *
 * У цьому проєкті таке вже було, і саме тому перевірка тут: `checkForUpdates()`
 * кличеться з `visibilitychange` і `focus`, тобто ЩОРАЗУ, коли до вкладки
 * повертаються. Висів не один запит, а по одному на кожне повернення, і
 * видимих наслідків це не давало — банер оновлення просто не з'являвся. Клас
 * дефектів, який нічим себе не виказує, і живе тому довго.
 *
 * Перевіряється наявність сигналу, а не його вид, і це навмисно. Канон
 * розрізняє два випадки за межею життя запиту: збігається з межею життя
 * ефекту — `getAbortSignal()` зі `svelte`; не збігається (кнопка «Скасувати»,
 * запит довший за компонент) — свій `AbortController` або
 * `AbortSignal.timeout()`. Єдиний `fetch` цього проєкту — другий випадок:
 * оновлення перепитується з обробника вкладки, а не з руни. Що саме доречно,
 * читанням джерела не встановити; що сигналу НЕМАЄ — встановити можна.
 *
 * Зворотний експеримент виконано: прибраний `signal` у `versionService.ts` —
 * червоне з іменем файлу й рядка.
 */

/** Виклик `fetch` разом з аргументами: дужки всередині рядків і об'єктів враховані одним рівнем. */
const FETCH_CALL = /\bfetch\s*\((?:[^()]|\([^()]*\))*\)/g;

const HAS_SIGNAL = /\bsignal\s*:/;

const fetchesWithoutSignal = SOURCES.flatMap((file) => {
	const src = srcOf(file);
	return [...src.matchAll(FETCH_CALL)]
		.filter((m) => !HAS_SIGNAL.test(m[0]))
		.map((m) => `${file}:${src.slice(0, m.index).split("\n").length} — ${m[0].replace(/\s+/g, " ").slice(0, 70)}`);
});

describe("запит у мережу має сигнал скасування (SC-ABORT-SIGNAL, MEDIUM)", () => {
	it("перевірка жива: виклики fetch у джерелах знайдено", () => {
		const calls = SOURCES.flatMap((f) => [...srcOf(f).matchAll(FETCH_CALL)]);
		expect(
			calls.length,
			"жодного fetch не знайдено — сканер зламався, і «порушень немає» " +
				"правдиве за побудовою",
		).toBeGreaterThan(0);
	});

	it("розбір бачить сигнал саме там, де він є", () => {
		// Обидва боки пари під гейтом: зійдуться — і перевірка мовчки помре.
		expect(HAS_SIGNAL.test('fetch(url, { signal: AbortSignal.timeout(5) })')).toBe(true);
		expect(HAS_SIGNAL.test('fetch(url, { cache: "no-store" })')).toBe(false);
	});

	it("кожен fetch передає signal", () => {
		expect(
			fetchesWithoutSignal,
			"`fetch` без `signal` не закінчується сам: з'єднання встановлено, " +
				"відповіді немає, і запит висить, доки браузер не здасться. Межа " +
				"життя ефекту — `getAbortSignal()`; своя межа — `AbortSignal.timeout()` " +
				"чи власний контролер:\n  " + fetchesWithoutSignal.join("\n  "),
		).toEqual([]);
	});
});
