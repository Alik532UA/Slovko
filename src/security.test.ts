...
describe("безпека", () => {
	 ...
	 */
	/**
	 * ОДИН файл має право містити ключ `AIza…`, і рівно один.
	 *
	 * Веб-ключ Firebase не є секретом: він приїжджає в кожну сторінку разом із
	 * бандлом, і сховати його неможливо в принципі. Межу безпеки тримають
	 * правила доступу й перелік дозволених доменів
	 * (SECURITY-v9 § 4.1, § 4.2.1 `SEC-CONFIG-IN-SOURCE`).
	 *
	 * Виняток прив'язаний до ШЛЯХУ, а не знятий загалом: `AIza…` у будь-якому
	 * іншому файлі — і далі порушення. Це важливо, бо той самий префікс мають
	 * ключі, які секретом Є: серверні ключі Google Cloud без обмеження за
	 * доменом роблять платні запити від імені власника.
	 *
	 * Решта форм — приватний ключ, сервісний акаунт, `api_secret` — не мають
	 * винятків ніде й ніколи.
	 */
	const PUBLIC_CONFIG = "src/lib/services/firebase/config.ts";
	...
	it("немає вписаного ключа чи токена в джерелах (§ 4.1)", () => {
		...
		/** Публічний веб-ключ Firebase — дозволений лише у `PUBLIC_CONFIG`. */
		...
		for (const file of sources) {
			...
			if (short(file) !== PUBLIC_CONFIG) {
				const m = GOOGLE_KEY.exec(text);
				if (m) bad.push(`${short(file)}: ${m[0].slice(0, 40)}…`);
			}
		}
	...
	/*
	 * Виняток вище мусить лишатися вузьким. Якщо файл переїде або зникне,
	 * `PUBLIC_CONFIG` почне вказувати в порожнечу — і тоді перевірка вище
	 * мовчки перетвориться на заборону без винятку, а наступний, хто її
	 * побачить, просто зніме перевірку цілком.
	 */
	it("виняток для публічного конфігу вказує на наявний файл (§ 4.2.1)", () => {
		expect(
			sources.map(short),
			`${PUBLIC_CONFIG} не знайдено серед джерел — виняток застарів`,
		).toContain(PUBLIC_CONFIG);
	});
