...
/**
 * `$_('ключ') || 'запас'` — запас, який не спрацює НІКОЛИ.
 *
 * `$_` віддає непорожній рядок завжди: або переклад, або сам ідентифікатор
 * ключа. `||` бере перше значення, тобто права половина недосяжна за
 * побудовою — незалежно від того, є ключ у словниках чи ні.
 *
 * Шкода не в мертвому рядку, а в тому, що він БРЕШЕ про надійність. Шість
 * місць, які показували ідентифікатор ключа замість тексту, простояли саме
 * тому: поруч із кожним стояв «запас», і код читався як передбачений. Знайти
 * їх очима було неможливо — саме там, де все виглядало найакуратніше.
 *
 * Прогін на момент появи зняв 69 таких запасів у сімнадцяти компонентах.
 * Захардкоджений текст у них був англійським або українським — тобто в
 * застосунку із сімома мовами він і не міг бути правильним відповідем.
 *
 * Правильний запас один — ключ у словниках, і його тримає перевірка вище.
 *
 * Зворотний експеримент виконано: повернутий `|| "Menu"` у `GameStats.svelte`
 * — червоне з іменем файлу й рядка.
 */
const DEAD_FALLBACK = /\$_\((?:[^()]|\([^()]*\))*\)\s*\|\|/;

const deadFallbacks = files.flatMap((file) => {
	const short = file.replace(`${ROOT.replace(/\\/g, "/")}/`, "");
	return [...withoutComments(readFileSync(file, "utf8")).split("\n").entries()]
		.filter(([, line]) => DEAD_FALLBACK.test(line))
		.map(([i, line]) => `${short}:${i + 1} — ${line.trim().slice(0, 80)}`);
});
...
describe("покриття локалізації", () => {
	...
	it("немає запасу `$_(…) || …`, який не спрацює ніколи", () => {
		expect(
			deadFallbacks,
			"`$_` віддає непорожній рядок завжди — або переклад, або сам " +
				"ідентифікатор ключа, — тож права половина `||` недосяжна. " +
				"Такий запас не рятує: він робить зламане місце схожим на " +
				"передбачене:\n  " + deadFallbacks.join("\n  "),
		).toEqual([]);
	});
});
