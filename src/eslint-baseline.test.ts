 ...
 *
 * Дванадцять адрес без `resolve()`. Правило лишається `warn`, бо разова зміна на
 * дванадцять місць непереглядна, — але з цієї хвилини число може лише спадати.
 *
 * Заодно прибрано мертву директиву `eslint-disable-next-line prefer-rest-params`
 * у `services/analytics.ts`: правила в конфігу немає, тож директива нічого не
 * вимикала, а `--report-unused-disable-directives` рахував її окремим
 * попередженням без `ruleId`. Той самий клас, що вимкнення, яке виглядає
 * зробленим і не зроблене.
 *
...
const DEBT: Readonly<Record<string, number>> = {
	'svelte/no-navigation-without-resolve': 12
};
...
describe('борг ESLint — число, що лише спадає (CODE-QUALITY-v8 § 6.4.3)', () => {
	...
	it('перевірка жива: lint пройшов по джерелах проєкту', () => {
		 ...
		 *
		 * 317 — заміряно прогоном на момент коміту. Межа вдвічі нижча: не
