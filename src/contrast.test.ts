 ...
 *
 * 189 → 185: `.wave-btn:hover` більше не блакитна. Тло на наведенні було
 * вписане числом (`#4a9fe6`) повз токени — помаранчева кнопка ставала синьою
 * в усіх чотирьох темах, — і давало 2,84. З `--accent-hover` виходить 4,95
 * у трьох темах і 5,48 у `green`.
 *
 * 185 → 177: `.skip-all-btn` більше не бере тлом `--status-success`. Той токен
 * підібраний як ТЕКСТ на звичайному тлі, і під білим написом давав найгіршу
 * пару проєкту — 2,10 : 1 у `orange`. Пішли ТРИ пари цілком
 * (`#ffffff` на `#2ecc71`, `#22c55e`, `#16a34a` — по два місця кожна), а
 * четверта, `#ffffff on #059669`, спала з 42 до 40: у темі `green` та сама
 * кнопка потрапляла в бучок акценту, бо `--status-success` там дорівнює
 * `--accent`. Нове тло — `--status-success-solid` (`#15803d`, 5,02 : 1 з
 * білим), і воно НЕ дає нової пари: перелік звіряється на рівність.
 */
const KNOWN_TOTAL = 177;
