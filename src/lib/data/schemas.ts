...
const AppSettingsCoreSchema = z.object({
	...
	/**
	 * Чи діють одиночні літерні скорочення (`T`, `L`).
	 *
	 * Це не зручність, а вимога WCAG SC 2.1.4 «Character Key Shortcuts», рівень
	 * A: скорочення з однієї літери мусить мати спосіб вимкнути, перепризначити
	 * або діяти лише у фокусі (HOTKEYS-v8 § 3). Кому це потрібно насправді —
	 * людям, які вводять текст голосом: диктування розсипається на одиночні
	 * літери, і кожна виконує команду.
	 *
	 * `default(true)` — тобто наявні збережені налаштування підхоплюють значення
	 * самі, без міграції. Службові жести (`V`, `R`) цим прапорцем НЕ керуються:
	 * вони не одиночні натискання, а серії, тож під критерій не потрапляють.
	 */
	enableHotkeys: z.boolean().default(true),
	updatedAt: z.number().default(0),
});
