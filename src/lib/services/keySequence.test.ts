 ...
 *
 * Це не абстрактна повнота: до цього рефакторингу серія `R` жила в
 * `LogCopyButton` і мала три обмеження з пʼяти. Тести на ВІКНО між натисканнями й
 * на МОДИФІКАТОРИ покривають рівно ті два, яких не було: без вікна лічильник жив
 * усю сесію (пʼять `R` за годину = скидання), а `Ctrl+R` рахувався кроком серії.
 */
...
describe("createKeySequence", () => {
	...
	it("спрацьовує рівно на порозі, не раніше", () => {
		...
		const sequence = createKeySequence({
			code: "KeyR",
			threshold: 3,
			onComplete,
		});
