 ...
 * у ЗАХИСТАХ, а захист, який ніхто не перевіряє, — це побажання. Тут цей жест
 * жив усередині `LogCopyButton` і мав ТРИ обмеження з пʼяти: не було ні вікна між
 * натисканнями (лічильник жив усю сесію, тож пʼять `R` за годину давали скидання),
 * ні перевірки модифікаторів (`Ctrl+R` рахувався кроком серії).
 *
 ...
 *     друкує в пошуку, у назві плейлиста чи у формі відгуку.
 *  3. Вікно між натисканнями. Без нього лічильник живе всю сесію.
 ...
 *     перезавантаження (HOTKEYS-v8 § 2.1).
 *
...
export function createKeySequence(options: KeySequenceOptions): KeySequence {
	...
	return {
		...
		handle(stroke: KeyStroke): void {
			...
			// Поріг читається НА КОЖНЕ натискання: він міг змінитися від
			// попереднього спрацювання цього ж жесту.
			const threshold =
