...
/**
 * Скільки чекати на `app-version.json`. Файл важить кілька десятків байтів і
 * лежить на тому самому хості, тож десять секунд — це вже точно не «повільно»,
 * а «не відповість». Менше брати не варто: у поїзді чи метро повільна, але
 * жива мережа не мусить читатися як обрив.
 */
...
export async function checkForUpdates() {
	...
	try {
		...
		response = await fetch(`${VERSION_URL}?t=${now}`, {
			cache: "no-store",
			/*
			 * Межа часу, бо «немає мережі» й «мережа є, відповіді немає» — різні
			 * випадки, і другий сам не закінчується.
			 *
			 * `navigator.onLine` вище ловить лише перший. Другий — це captive
			 * portal у готелі чи кафе, мертвий проксі, вимкнений посеред запиту
			 * тунель: з'єднання встановлюється, і `fetch` висить, доки браузер не
			 * здасться сам (у Chrome це десятки секунд, а в частині випадків не
			 * настає ніколи). Перевірка викликається з `visibilitychange` і
			 * `focus`, тобто щоразу, коли до вкладки повертаються, — тож висів би
			 * не один запит, а по одному на кожне повернення.
			 *
			 * Видимих наслідків це не давало (банер оновлення просто не
			 * з'являвся), і саме тому могло жити довго. Відмова за часом
			 * потрапляє в той самий `catch` нижче, що й будь-який мережевий збій,
			 * і так само не інкрементує лічильник помилок.
			 */
