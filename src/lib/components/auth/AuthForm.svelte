...
<style>
	...
	/*
	 * Текст бере `--text-on-card`, а не `--accent`.
	 *
	 * Акцент — токен, ПІДІБРАНИЙ ЯК ТЛО: під ним стоїть `--text-on-accent`.
	 * Ужитий навпаки, текстом на картці, він давав 2,58 : 1
	 * (`#e95420` на `#43464e`) — це був останній вузол `color-contrast`, який
	 * axe бачив на стані `profile`, окрім самого акценту.
	 *
	 * `contrast.test.ts` цю пару не бачить і не побачить: він зіставляє `color`
	 * із `background`, оголошеними в ОДНОМУ блоці, а тут `background: none` і
	 * тло приходить успадкуванням від картки. Тобто клас «колір без тла»
	 * лишається за axe, і саме тому пара названа тут словами.
	 *
	 * `--text-on-card` дає 13,6 : 1 у світлій темі, 7,3 у темній і більше в
	 * `orange` та `green` — це і є токен для тексту на цій поверхні.
	 *
	 * Підкреслення СТАЛЕ, а не на наведенні. Доти єдиною ознакою посилання був
	 * колір (WCAG 1.4.1): без наведення відрізнити його від звичайного тексту
	 * не міг ніхто, хто не бачить різниці відтінків. Наведення тепер міняє
	 * товщину й колір ЛІНІЇ — на контраст тексту це не впливає, а акцент як
	 * мова взаємодії лишається.
	 *
	 * Рядок коду тут один доданий, і він коштував підняття стелі цього файлу в
	 * `file-size.test.ts` (450 → 451): запас там нульовий за побудовою. Правильна
	 * плата — розділити файл за відповідальністю; вона лишається боргом.
	 */
	.btn-link {
		background: none;
		border: none;
