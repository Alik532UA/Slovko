...
<style>
	...
	.segment-btn {
		...
		z-index: 1;
		/*
		 * Розмір ведений шириною вікна, і це єдиний доступний важіль.
		 *
		 * `white-space: nowrap` нижче — свідома вимога (AC 4.1), а
		 * `min-width: max-content` робить із неї жорстку підлогу: кнопка не
		 * може стати вужчою за свій напис. Довгі українські слова
		 * («Однотонний», «Зображення») при фіксованих 0.85rem розпирали
		 * контрол за межі модалки на вузькому екрані.
		 *
		 * Раз текст не можна ні переносити, ні обрізати — зменшується він сам,
		 * а разом із ним і `max-content`.
		 */
