<script lang="ts">
	 ...
	 *
	 * Тепер олівець на місці, але СІРИЙ, а під курсором стає замком і показує
	 * причину словами. Німий `if` лишився другим шаром, бо недоступність,
	 * намальована лише стилями, тримається доти, доки хтось не покличе обробник
	 * іншим шляхом.
	 *
	 * ТАМ, ДЕ НАВЕДЕННЯ НЕМАЄ (телефон), підміна не спрацює ніколи, а підказку
	 * `BaseTooltip` до 768px не показує зовсім. Тому за межами `@media (hover:
	 * hover)` стоїть відразу замок: інакше на телефоні лишився б сірий олівець
	 * без жодного слова — та сама скарга, з якої все почалося.
	 *
		...
		<BaseTooltip text={$_("profile.editLocked")}>
			<div class="face-box is-locked" data-testid="avatar-locked-status">
				...
				<div class="edit-overlay is-locked" aria-hidden="true">
					<span class="swap-pen"><Edit2 size={16} /></span>
					<span class="swap-lock"><Lock size={16} /></span>
				</div>
			</div>
		</BaseTooltip>
	{:else if mode === "edit"}
...
<style>
	...
	/*
	 * ЗНАЧОК ВИДНО В СПОКОЇ — обидва, і в власника, і в гостя.
	 *
	 * Доти олівець власника проявлявся лише під курсором, а замок гостя стояв
	 * завжди — тобто замкнений стан рекламував себе гучніше за робочий, і на
	 * дотиковому екрані власник не бачив узагалі нічого. Тепер різницю несе
	 * колір: акцентний у власника, сірий у гостя.
	 */
	.edit-overlay {
		position: absolute;
		bottom: 0;
	...
	/*
	 * ПІДМІНА ОЛІВЦЯ ЗАМКОМ.
	 *
	 * ТИПОВИЙ стан — замок, і саме так це правильно там, де наведення не існує:
	 * на телефоні підміна не спрацювала б ніколи, а `BaseTooltip` до 768px не
	 * показується зовсім, тож сірий олівець лишився б там без жодного слова.
	 * Олівець вертає `@media (hover: hover)` — тим, у кого є курсор і кому є що
	 * навести.
	 *
	 * Фокус із клавіатури значків не міняє, і це не пропуск: підказку
	 * `BaseTooltip` показує сама на `focusin`, тобто причина приходить словами, а
	 * не малюнком.
	 */
	.swap-pen {
		display: none;
	...
	@media (hover: hover) {
		...
		.face-box.is-locked:hover .swap-pen {
			display: none;
		}

		.face-box.is-locked:hover .swap-lock {
			display: flex;
		}
	}
</style>
