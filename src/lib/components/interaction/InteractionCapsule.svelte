...
>
	<!--
		Обґрунтування — і воно НЕ «тут усе гаразд», а названа межа.

		Розгортання капсули справді доступне лише мишкою: `role="button"` із
		`tabindex` тут поставити не можна, бо всередині вже лежить справжня
		`<button>` дії, а кнопка в кнопці — це і невалідна розмітка, і зламаний
		порядок фокуса. Правильне лікування — розділити «розгорнути» й «виконати
		дію» на два окремі контроли, тобто перебудувати компонент, а не додати
		атрибут.

		Що доступне з клавіатури зараз: сама дія (кнопка всередині) — після того,
		як капсулу розгорнули, і закриття Escape. Недоступне: розгортання типів
		`manual_menu` і `new_follower`.

		Записано як борг у PROJECT-CONTEXT.md, бо мовчазний `svelte-ignore`
		читається як «перевірено й нормально», а це неправда.
	-->
...
<style>
	...
	/*
	 * Кнопка «привітатися». Тло — акцент теми, наведення — `--accent-hover`,
	 * тобто той самий токен, що й у решти акцентних кнопок проєкту.
	 *
	 * Доти в спокої стояв `var(--accent, #3a8fd6)`, а на наведенні —
	 * `#4a9fe6`: помаранчева кнопка ставала БЛАКИТНОЮ, і однаково в усіх
	 * чотирьох темах, бо число написане повз токени. Заразом це була друга
	 * найгірша пара контрасту проєкту — білий на `#4a9fe6` дає 2,84 при
	 * потрібних 4,5. З `--accent-hover` виходить 4,95 у трьох темах і 5,48 у
	 * `green`.
	 *
	 * Запасне значення в `var()` прибрано разом із ним: `--accent` оголошений
	 * у кожній темі (це стоїть канаркою в `contrast.test.ts`), тож запасне
	 * лише ховало б помилку в імені.
	 */
	.wave-btn {
