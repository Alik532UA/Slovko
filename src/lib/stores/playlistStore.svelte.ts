...
function createPlaylistStore() {
    ...
    return {
        ...
        removeFromFavorites(pairId: string) {
            state = { ...state, favorites: state.favorites.filter(p => p.id !== pairId) };
            saveState();
        },
        ...
        recordMistake(pair: WordPair) {
            ...
            const existingIdx = state.mistakes.findIndex(m => m.pair.id === pair.id);
            if (existingIdx >= 0) {
                ...
                // Add new
                state = { ...state, mistakes: [...state.mistakes, { pair, correctStreak: 0 }] };
            }
            saveState();
        },
        ...
        recordCorrect(pairId: string) {
            ...
            if (existingIdx >= 0) {
                ...
                // Check threshold
                if (newMistakes[existingIdx].correctStreak >= MISTAKE_REMOVAL_THRESHOLD) {
                    // Remove
