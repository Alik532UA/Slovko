...
export class WebVitals {
	...
	start(): () => void {
		...
		this.#observe('largest-contentful-paint', (entries) => {
			...
			if (last) logService.info('debug', `[Performance] LCP: ${last.startTime.toFixed(0)}ms`);
		});
		...
		this.#observe('layout-shift', (entries) => {
			...
			logService.info('debug', `[Performance] CLS: ${this.#cls.toFixed(4)}`);
		});

		this.#observe('event', (entries) => {
			const worst = Math.max(...entries.map((e) => e.duration));
			if (worst > 0) logService.info('debug', `[Performance] INP: ${worst.toFixed(0)}ms`);
		});
	...
	#observe(type: string, handler: (entries: PerformanceEntry[]) => void) {
		try {
			...
			logService.warn('debug', `PerformanceObserver does not support "${type}"`);
		}
	}
}
