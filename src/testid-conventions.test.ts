...
describe("data-testid conventions (v8)", () => {
	...
	it("у списку дозволених дублікатів немає записів про неіснуючі id", () => {
		/*
		 * Список винятків, який перелічує те, чого в коді немає, — це дозвіл,
		 * виданий у порожнечу: він нічого не пропускає, зате розповідає читачеві
		 * про елемент, якого нема, і про причину, якої вже не існує.
		 *
		 * Реальний випадок: `change-password-back-btn` стояв тут із поясненням
		 * «одна й та сама кнопка «назад» у гілках google-акаунта і звичайної
		 * форми», тоді як самої форми зміни пароля в проєкті не було — від неї
		 * лишився тільки цей запис (форму повернули 2026-08-19).
		 *
		 * Список за каноном може лише СКОРОЧУВАТИСЯ (§ 1.10) — ця перевірка
		 * робить скорочення обов'язковим, а не бажаним.
		 */
		...
		const stale = [...ALLOWED_DUPLICATES].filter(
			(id) => ![...counts].some(([key, n]) => key.endsWith(`::${id}`) && n > 1),
		);
		expect(
			stale,
			`дозвіл виданий id, який ніде не дублюється:\n${stale.join("\n")}`,
		).toEqual([]);
	});
});
