...
describe("хмарна база", () => {
	...
	/**
	 * ВИДАЛЕННЯ АКАУНТА ПРИБИРАЄ КОЖНУ ПІДКОЛЕКЦІЮ, ЯКУ ЗНАЮТЬ ПРАВИЛА.
	 *
	 * Firestore НЕ видаляє підколекції разом із документом, а після
	 * `deleteUser()` їх не може прибрати вже ніхто: правила вимагають власника.
	 * Тому забута підколекція означає дані, які лишаються назавжди, — і побачити
	 * це в коді неможливо: `deleteDoc(users/{uid})` виглядає повним прибиранням.
	 *
	 * Перевірка йде ВІД ПРАВИЛ до коду, а не навпаки: правила — єдине місце, де
	 * перелічені всі підколекції користувача. Нова підколекція без рядка в
	 * `EraseService` тепер валить прогін, а не тихо переживає видалення акаунта.
	 */
	it("видалення акаунта прибирає кожну підколекцію користувача", () => {
		const erase = readFileSync("src/lib/services/firebase/EraseService.ts", "utf8");

		/*
		 * Блок `users/{uid}` вирізається ДВОМА МІТКАМИ, а не регуляркою через увесь
		 * файл: вкладені фігурні дужки правил регулярним виразом не балансуються, і
		 * «до першої закритої» обірвало б блок на першій же підколекції — тобто
		 * перевірка зеленіла б, побачивши одну з п'яти.
		 */
		const start = rulesCode.indexOf("match /users/{uid}");
		const end = rulesCode.indexOf("match /profiles/{uid}");
		expect(start, "блок правил users/{uid} не знайдено — перевірка мертва").toBeGreaterThan(-1);
		expect(end, "мітка кінця блоку зникла — перевірка осліпла").toBeGreaterThan(start);

		const subcollections = [
			...rulesCode.slice(start, end).matchAll(/match\s+\/([a-z_][\w-]*)\/\{/gi)
		].map((m) => m[1]);
		expect(subcollections.length, "підколекцій не знайдено — перевірка мертва").toBeGreaterThan(3);

		const forgotten = subcollections.filter((name) => !erase.includes(name));
		expect(forgotten, `підколекція переживе видалення акаунта: ${forgotten.join(", ")}`).toEqual(
			[]
		);

		// Головні документи теж мусять зникати — інакше перевірка вище зеленіла б
		// на сервісі, що прибирає лише підколекції.
		expect(erase, "публічний профіль не прибирається").toMatch(/"profiles"/);
		expect(erase, "документ користувача не прибирається").toMatch(/"users"/);
	});
